...
def main():
    ...
    train_dataset = datasets.FashionMNIST(root=config.data_root, train=True, download=True, transform=transform)
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=config.train_batch_size, shuffle=True, num_workers=4, pin_memory=True
    )

    test_dataset = datasets.FashionMNIST(root=config.data_root, train=False, download=True, transform=transform)
    test_dataloader = torch.utils.data.DataLoader(
        test_dataset, batch_size=config.test_batch_size, shuffle=True, num_workers=4, pin_memory=True
    )

    model = VAE(image_size=config.image_size)
    optimizer = Adam(model.parameters(), lr=0.0002, betas=(0.5, 0.999))
    trainer = Trainer(model, train_dataloader, test_dataloader, optimizer, loss_function, device='cpu')
    ...
    test_dataset = datasets.FashionMNIST(root=config.data_root, train=False, download=True, transform=transform)
    test_dataloader = torch.utils.data.DataLoader(
        test_dataset, batch_size=5000, shuffle=True, num_workers=4, pin_memory=True
    )
